            ...
            if (opcode.length() == 1) {
                ...
                switch (opcode.substring(opcode.length() - 2)) {
                    ...
                    case "05": {
                        int mode = parseNumber(opcode, opcode.length() - 3);
                        String condition = "";
                        if (mode == 0) condition = parts[Integer.parseInt(parts[position + 1])];
                        else condition = parts[position + 1];
                        if (!condition.equalsIgnoreCase("0")) {
                            position = Integer.parseInt(parts[Integer.parseInt(parts[position + 2])]);
                            System.out.println("J-i-T (Mode " + mode + "): " + condition + ": jumped to " + position);
                        } else {
                            position += 2;
                            System.out.println("J-i-T (Mode " + mode + "): " + condition + ": Nothing happened");
                        }
                        break;
                    }
                    case "06": {
                        ...
                        if (condition.equalsIgnoreCase("0")) {
                            System.out.println("J-i-F (Mode " + mode + "): " + condition + ": jumped to " + position);
                            position = Integer.parseInt(parts[Integer.parseInt(parts[position + 2])]);
                        } else {
                            position += 2;
                            System.out.println("J-i-F (Mode " + mode + "): " + condition + ": Nothing happened");
                        }
                        break;
                    }
                    case "07": {
