...
public class Day7A {
    ...
    private static int getMax(int a, int b, int c, int d, int e) {
        IntcodeParser parser = new IntcodeParser(input);
        int number = 0;
        number = parser.parseCode(a, number);
        number = parser.parseCode(b, number);
        number = parser.parseCode(c, number);
        number = parser.parseCode(d, number);
        return parser.parseCode(e, number);
    }
}
