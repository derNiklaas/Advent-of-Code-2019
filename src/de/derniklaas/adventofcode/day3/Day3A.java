...
public class Day3A {
            ...
            switch (part.charAt(0)) {
                case 'U': { // UP -> y-1
                    for (int i = 0; i < amount; i++) {
                        ...
                        if (!crossing) {
                            wires.add(new Point(current.getX(), current.getY()));
                        } else {
                            if (testForWire(current)) {
                                crossings.add(new Point(current.getX(), current.getY()));
    ...
    private static boolean testForWire(Point p) {
