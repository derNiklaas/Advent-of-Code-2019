...
public class Day3A {
    ...
    // I assume that the field is 1k x 1k
    //private static int[][] crossings = new int[30000][30000];
    private static int startX = 15000, startY = 15000;
    private static List<Point> wires = new ArrayList<>();
    private static List<Point> crossings = new ArrayList<>();

    public static void main(String[] args) {
        //I assume that the starting point of each thing is 1000 | 1000
        String wire1 = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51";
        String wire2 = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";
        enterWire(wire1);
        enterWire(wire2);
        Point closest = new Point(30001, 30001);
        for (Point p : crossings) {
            if (calcDistance(startX, startY, p.getX(), p.getY()) < calcDistance(startX, startY, closest.getX(), closest.getY())) {
                closest = p;
            }
        }
        System.out.println("-----");
        System.out.println("Closest distance: " + calcDistance(startX, startY, closest.getX(), closest.getY()));
    }

    private static void enterWire(String wire) {
        int x = startX;
        int y = startY;
            ...
            switch (part.charAt(0)) {
                case 'R': {
                    ...
                    for (int i = 0; i <= amount; i++) {
                        x++;
                        if (containsWire(x, y)) {
                            crossings.add(new Point(x, y));
                            System.out.println("New crossing at: " + x + "|" + y + " Distance: " + calcDistance(startX, startY, x, y));
                        } else wires.add(new Point(x, y));
                    }
                    break;
                }
                case 'D': {
    ...
    private static boolean containsWire(int x, int y) {
        for (Point p : wires) {
    ...
    private static int calcDistance(int startX, int startY, int stopX, int stopY) {
        return Math.abs(startX - stopX) + Math.abs(startX - stopY);
    }
}
